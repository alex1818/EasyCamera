...
public:
  ...
  typedef bool (u10ResponseHandler)(char* responseStr, unsigned responseStrLen, char* cseq, char* location);

  u10ResponseHandler* fRedirectHandler;
  u10ResponseHandler* fPauseHandler;

  void SetRTSPClientRedirectHandler(u10ResponseHandler* handler)
  {
	  fRedirectHandler = handler;
  }

  void SetRTSPClientPauseHandler(u10ResponseHandler* handler)
  {
	  fPauseHandler = handler;
  }
  ...
  char const* puid() const { return fPuid; }
...
protected:
  RTSPClient(UsageEnvironment& env, char const* rtspURL, char const* puid,
	     int verbosityLevel, char const* applicationName, portNumBits tunnelOverHTTPPortNum);
      // called only by createNew();
  virtual ~RTSPClient();

  void setBaseURL(char const* url, char const* puid);
