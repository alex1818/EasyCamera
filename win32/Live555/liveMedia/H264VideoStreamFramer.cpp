...
{   
    ...
    {
        printf("Create camera instance error");
        return NULL;
    }
    printf("m_pCamera->OpenCamera\n");
	// 打开Camera
    ...
    {
        printf("Initialize H.264 decoder error.");
        return NULL;
    }
    ...
    if(NULL == g_IplImage)
    {
        printf("Initialize OpenCV error.");
        return NULL;
    }
#endif
...
void CamH264VideoStreamFramer::doGetNextFrame()
{
    ...
    while(len > 0)
    {
        ...
        if(bGetFrame)
        {
            ...
            cvShowImage("Preview", g_IplImage);
            cvWaitKey(5);
#endif       
...
#endif       
            //printf("Success to decode one frame[%d]", iDecodedFrame);
            iDecodedFrame++;
        }
        len -= iDecodedLen;
        nal_ptr += iDecodedLen;
